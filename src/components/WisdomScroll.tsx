...
// How far above the viewport the paper top is anchored (px)
const ABOVE = 220;
...
export default function WisdomScroll({ onClose, language }: WisdomScrollProps) {
  ...
  return (
    ...
    >
      {/* Container anchored ABOVE px above the viewport — paper top is always off-screen */}
      <div
        className="relative w-full max-w-3xl flex flex-col items-center"
        style={{ marginTop: `-${ABOVE}px` }}
      >
        {/* Scroll body: height grows from 0 → 85vh+ABOVE, so the roller falls from off-screen to the bottom */}
        <motion.div
          initial={{ height: 0 }}
          animate={{ height: `calc(90vh + ${ABOVE}px)` }}
          exit={{ height: 0 }}
          transition={{ duration: 1.6, ease: [0.22, 1, 0.36, 1], delay: 0.15 }}
          className="relative w-full flex flex-col items-center z-10"
        >
          {/* overflow-hidden clips the growing paper reveal */}
          <div className="w-full h-full overflow-hidden relative flex justify-center">
            {/* Fixed-height inner so paper and content don't squish during animation */}
            <div
            ...
            >
              ...
              {/* Content — padded by ABOVE so it starts inside the visible viewport */}
              <div
                className="relative z-10 w-[86%] md:w-[80%] h-full flex flex-col pb-20 md:pb-28"
                style={{ paddingTop: `${ABOVE + 100}px` }}
              >
                {/* Header */}
                <div className="flex justify-between items-center px-4 py-2 md:px-6 md:py-4 border-b border-[#2b1d0f]/20">
                  <h2
                    className="text-xl md:text-3xl font-serif text-[#3e2723] font-bold tracking-widest uppercase"
                    ...
                    <AnimatePresence>
                      {wisdoms.map((wisdom, i) => (
                        ...
                        >
                          ...
                          <p
                            className="text-[#1a0f0a] font-serif text-lg md:text-xl leading-relaxed mb-6 font-bold"
          ...
          {/* Bottom roller — starts off-screen (height=0 → roller at -10px above container) and falls */}
